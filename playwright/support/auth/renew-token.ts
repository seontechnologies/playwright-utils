...
export async function renewToken(storageState: StorageStateType) {
  ...
  const storageStateContent = await context.storageState()
  if (!storageStateContent.cookies || storageStateContent.cookies.length < 1) {
    throw new Error('No cookies found after renewing the token')
  }

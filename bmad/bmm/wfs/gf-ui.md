...
```xml
<workflow id="bmad/bmm/wfs/gf-ui.md" name="Greenfield UI/Frontend Development" type="greenfield">
  <meta>
    <desc>Agent workflow for building frontend applications from concept to development. Supports comprehensive planning for complex UIs and rapid prototyping.</desc>
    <types>spa, mobile-app, micro-frontend, static-site, ui-prototype, simple-interface</types>
  </meta>
  ...
  <steps>
    ...
    <step agent="pm" creates="prd.md" needs="project-brief.md">
      <notes>Create PRD from brief using prd cmd, focus UI/frontend. SAVE: {project-root}{output-directory}/prd.md</notes>
    </step>
    ...
    <step agent="ux-expert" creates="ai_ui_prompt.md" needs="front-end-spec.md" if="user_wants_ai_generation">
      <notes>OPTIONAL: Generate AI UI prompt (v0/Lovable) using ai-fe-prompt task</notes>
    </step>

    <step agent="architect" creates="front-end-architecture.md" needs="front-end-spec.md">
      <opts>technical_research_prompt; review_generated_ui_structure</opts>
      <notes>Create frontend architecture. May suggest PRD changes. SAVE: {project-root}{output-directory}/front-end-architecture.md</notes>
    </step>

    <step agent="pm" updates="prd.md" needs="front-end-architecture.md" if="architecture_suggests_prd_changes">
    ...
    <step type="guide" action="guide_project_structure" if="user_has_generated_ui">
      <notes>Polyrepo: separate frontend repo | Monorepo: apps/web or frontend/</notes>
    </step>
  ...
  <guidance>
    <use-when>Production frontends | Multi-view/complex interactions | UI/UX testing needed | Team project | Long-term maintenance | Customer-facing</use-when>
  </guidance>
  ...
  <handoffs>
    ...
    <h type="architect-review">Architecture done. Save {project-root}{output-directory}/front-end-architecture.md. Suggest PRD changes?</h>

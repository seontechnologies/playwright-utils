...
The Test Architect (Murat) provides comprehensive quality assurance throughout the development lifecycle. Here's how to leverage each capability at the right time.
...
| **After Development**    | `*review`   | Comprehensive assessment                | TEA Results in story + `docs/qa/gates/{epic}.{story}-{slug}.yml` | **Required**                |
...
4. **Gate Decision**
   ...
   - Adds: TEA Results section to story file
...
@tea *risk {story}    # First - identify dangers
@tea *design {story}  # Second - plan defense
# Then during dev:
@tea *trace {story}   # Verify regression coverage
@tea *nfr {story}     # Check performance impact
# Finally:
...
Murat ensures all tests meet these standards:
...
- **Story Updates**: TEA Results sections in story files

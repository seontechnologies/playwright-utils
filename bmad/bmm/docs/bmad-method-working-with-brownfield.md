...
> - If working in the web: use `npx bmad flatten` to flatten your project into a single file, then upload that file to your web agent.
...
1. **Follow the [<ins>User Guide - Installation</ins>](../bmad-modules/bmm/docs/user-guide.md#installation) steps to setup your agent in the web.**
2. **Generate a 'flattened' single file of your entire codebase** run: `npx bmad flatten`
...
4. **Run documentation**: Type `*doc-proj`
...
- Run `doc-proj` to capture current state
...
In brownfield projects, the Test Architect (Murat) becomes your safety net against breaking existing functionality. Unlike greenfield where you're building fresh, brownfield requires careful validation that new changes don't destabilize what already works.
...
@tea *review {brownfield-story}
# Performs: Deep analysis + active refactoring
# Outputs:
#   - TEA Results in story file
...
Murat enforces additional standards for brownfield:
...
3. **Test Architect Early Involvement**:
   - Run `@tea *risk` on draft stories to identify integration risks
   - Use `@tea *design` to plan regression test strategy
4. Architecture emphasizes compatibility
5. Stories include integration tasks with test requirements
6. **During Development**:
   - Developer runs `@tea *trace` to verify coverage
   - Use `@tea *nfr` to monitor performance impact
7. **Review Stage**: `@tea *review` validates integration safety
...
3. **Test Architect Strategy Planning**:
   - `@tea *risk` assesses modernization complexity
   - `@tea *design` plans parallel testing approach
...
6. **Continuous Validation**: Run `@tea *trace` after each increment
7. **Gate Management**: Use `@tea *gate` to track technical debt acceptance

### Scenario 3: Bug Fix in Complex System

1. Document relevant subsystems
2. Use `mk-bf-story` for focused fix
3. **Test Architect Risk Assessment**: Run `@tea *risk` to identify side effect potential
4. Include regression test requirements from `@tea *design` output
5. **During Fix**: Use `@tea *trace` to map affected functionality
6. **Before Commit**: Run `@tea *review` for comprehensive validation
...
3. **Test Architect Contract Analysis**:
   - `@tea *risk` identifies breaking change potential
   - `@tea *design` creates contract test strategy
...
7. **Validation Checkpoints**:
   - `@tea *trace` maps all API consumers
   - `@tea *nfr` validates response times
   - `@tea *review` ensures no breaking changes
...
**Solution**: Re-run `doc-proj` with more specific paths to critical files
...
**Solution**: Use `mk-bf-story` instead of full workflow
...
@tea *risk {story}     # Assess regression & integration risks
@tea *design {story}   # Plan regression + new feature tests

# DURING DEVELOPMENT (Validation)
@tea *trace {story}    # Verify coverage of old + new
@tea *nfr {story}      # Check performance degradation

# AFTER DEVELOPMENT (Review)
@tea *review {story}   # Deep integration analysis
@tea *gate {story}     # Update quality decision
```

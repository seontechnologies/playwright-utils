...
```mermaid
graph TD
    A["Development Phase Start"] --> A2["PO: Create Epics from Sharded PRD"]
    A2 --> A3["PO: Create Initial Stories from Epics"]
    A3 --> B["SM: Reviews Previous Story Dev/QA Notes"]
    B --> B1["SM: Review Story with story-checklist"]
    B1 --> B2["SM: Prepare Development-Ready Story with draft"]
    B2 --> B3["SM: Final Validation with story-checklist"]
    B3 --> S{"High-Risk Story? (Optional)"}
    S -->|Yes| T["QA: *risk + *design on Story"]
    S -->|No| B4
    T --> U["Test Strategy & Risk Profile Created"]
    U --> B4{"PO: Validate Story (Optional)"}
    B4 -->|Validation Requested| B5["PO: Validate Story Against Artifacts"]
    B4 -->|Skip Validation| C{"User Approval"}
    B5 --> C
    ...
    style B3 fill:#e8f5e9,color:#000
    style S fill:#e3f2fd,color:#000
    style T fill:#ffd54f,color:#000
    style U fill:#ffd54f,color:#000
    style B4 fill:#e3f2fd,color:#000
    style B5 fill:#fce4ec,color:#000
...
## The Scrum Master (SM Agent)

### SM Overview

The SM agent in BMad serves as the bridge between business requirements and technical implementation. Named Bob, this agent specializes in story preparation, taking Product Owner epics and stories and transforming them into development-ready specifications that eliminate ambiguity and enable efficient development.

#### SM Quick Start (Essential Commands)

```bash
@sm story-checklist {story}    # Review story readiness (first step)
@sm draft {story}              # Prepare development-ready story
@sm story-checklist {story}    # Final validation before handoff
@sm correct-course            # Execute course correction during process
@sm retrospective            # Facilitate team retrospective after epic/sprint
```

#### Command Usage Order

The commands are ordered by typical usage flow:

1. **story-checklist** - Quality gate to review PO stories and identify gaps
2. **draft** - Transform stories into development-ready specifications
3. **story-checklist** - Final validation before developer handoff
4. **correct-course** - Process correction when needed
5. **retrospective** - Post-sprint/epic team facilitation

### SM Core Capabilities

#### 1. Story Quality Checking (`story-checklist`)

**When:** First step when receiving PO stories, and final validation before dev handoff

Reviews story readiness including:

- Clear acceptance criteria existence
...
#### 2. Story Preparation (`draft`)

**When:** After identifying gaps through story-checklist

Transforms PO stories into development-ready specifications:

- Adds technical specifications and implementation details
- Creates granular, actionable tasks
- Defines clear Definition of Done criteria
- Identifies dependencies and constraints
- Prepares detailed handoff documentation

#### 3. Process Management

- **correct-course** - Execute mid-process corrections when workflow gets off track
- **retrospective** - Facilitate team retrospectives for continuous improvement

### Working with the Scrum Master

#### SM Integration with BMad Workflow

The SM provides the critical bridge between planning and development:

| **Stage**             | **Command**       | **When to Use**          | **Value**                       |
| --------------------- | ----------------- | ------------------------ | ------------------------------- |
| **Story Receipt**     | `story-checklist` | When PO provides stories | Identify preparation needs      |
| **Story Preparation** | `draft`           | After gap analysis       | Create dev-ready specifications |
| **Final Validation**  | `story-checklist` | Before dev handoff       | Ensure readiness                |
| **Process Issues**    | `correct-course`  | When workflow blocked    | Get back on track               |
| **Sprint End**        | `retrospective`   | After epic/sprint        | Continuous improvement          |

#### Typical Workflow

```text
PO Stories → SM story-checklist → Identify gaps → SM draft → Final story-checklist → Dev handoff
```

The SM ensures developers receive clear, unambiguous specifications that enable immediate productive work without mid-sprint questions or rework.

## The Test Architect (QA Agent)

### QA Overview

The TEA agent in BMad is not just a "senior developer reviewer" - it's a **Test Architect** with deep expertise in test strategy, quality gates, and risk-based testing. Named Murat, this agent provides advisory authority on quality matters while actively improving code when safe to do so.

#### QA Quick Start (Essential Commands)
...
#### QA Integration with BMad Workflow
...
*review        → TEA Results section in story + gate file reference
